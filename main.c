...
void ARRAY_INSERT_MIDDLE(struct student** TARGET_ARRAY_PTR, int* TARGET_ARRAY_SIZE, const struct student* INSERTED_ELEMENT, int WANTED_INDEX);
void ARRAY_INSERT_BEGINNING(struct student** TARGET_ARRAY_PTR, int* TARGET_ARRAY_SIZE, const struct student* INSERTED_ELEMENT);
void ARRAY_INSERT_END(struct student** TARGET_ARRAY_PTR, int* TARGET_ARRAY_SIZE, const struct student* INSERTED_ELEMENT);
...
//PASSING THE POINTER OF THE FIRST ELEMENT OF THE ARRAY BY REFERENCE AND THE OLD SIZE AND THE NEW ELEMENT
{
	...
	if (temp_array == NULL)  // CHECKING IF THE NEW ARRAY IS ALLOCATED OR NOT
		...
		int i;
		for (i = 0; i < *TARGET_ARRAY_SIZE - 1; i++) // COPYING THE OLD ARRAY
		{
...
//PASSING THE POINTER OF THE pointer of the FIRST ELEMENT OF THE ARRAY AND THE OLD SIZE AND THE NEW ELEMENT AND THE WANTED INDEX TO BE INSERTED
{
	if (WANTED_INDEX >= *TARGET_ARRAY_SIZE || WANTED_INDEX < 1)//CHECKING IF THE WANTED INDEX IS IN THE PERMITTED RANGE [1,SIZE[ (NOT IN BEGINNING OR END)
	{
		...
		// IF ALLOCATED INSERT THE ELEMENT IF NOT THEN DONT INSERT
			...
			int i;
			for (i = 0; i < WANTED_INDEX; i++)// INSERT THE OLD ELEMENTS BEFORE THE WANTED INDEX
			{
				*(temp_array + i) = *(*TARGET_ARRAY_PTR + i);
			}
			*(temp_array + WANTED_INDEX) = *INSERTED_ELEMENT;  // INSERT THE NEW ELEMENT
			for (i = WANTED_INDEX + 1; i < *TARGET_ARRAY_SIZE; i++) // INSERT THE OLD ELEMENTS AFTER THE WANTED ELEMENTS AND INDEX SHIFTED BY 1
			{
